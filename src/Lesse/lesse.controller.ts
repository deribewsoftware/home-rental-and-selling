...
export class LesseController{
    ...
    @Patch('house/:houseId')
    CreateDealing(@GetUser() authorId:User,@Param('houseId',ParseIntPipe) houseId:number,@Body() dealing:CreateDealingHouseDto){
        return this.lesseService. CreateDealing(authorId['sub'],houseId, dealing);
    }
